    ...
    <div class="tile-body">
        <form class="form-horizontal widget-settings-form" id="form-widget-panel" action="{{ route('widget.save', $widget) }}">
            ...
            <div class="form-group row">
                <label class="control-label col-md-4" for="copyright">Авторское право:</label>
                <div class="col-md-8">
                    <input
                            id="copyright"
                            name="copyright"
                            ...
                            type="text"
                            value="{{ $widget_setting->copyright }}">
                </div>
            </div>

            <div id="accordionExample" class="accordion">
                @foreach(data_get($widget_setting, 'items', []) as $setting)
                    @include('miracle.widgets.CopyrightArea.block', ['position' => $loop->iteration])
                @endforeach
            </div>
